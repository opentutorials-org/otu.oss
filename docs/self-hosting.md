...
> **참고**: `sudo`로 실행 시 `$HOME`이 `/root`으로 변경될 수 있습니다. 특정 디렉토리에 설치하려면 `OTU_INSTALL_DIR` 환경변수를 설정하세요.
...
| `MIGRATION_FAIL_FAST`      | 마이그레이션 실패 시 앱 시작 차단   | `false` |
