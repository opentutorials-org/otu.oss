...
if [ -d "$MIGRATIONS_DIR" ]; then
    ...
    for f in "$MIGRATIONS_DIR"/*.sql; do
        ...
        # set -e 환경에서 psql 실패 시 스크립트가 즉시 종료되지 않도록
        # || true로 보호하고 exit code를 별도 변수로 캡처
        MIGRATION_ERR=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" \
            -v ON_ERROR_STOP=1 -f "$f" 2>&1) && MIGRATION_RC=0 || MIGRATION_RC=$?
        if [ "$MIGRATION_RC" -eq 0 ]; then
            # 성공 시 기록 — FILENAME을 psql -v 파라미터 바인딩으로 전달 (SQL injection 방지)
            psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -q \
                -v version="$VERSION" -v name="$FILENAME" \
                -c "INSERT INTO supabase_migrations.schema_migrations (version, name) VALUES (:'version', :'name') ON CONFLICT DO NOTHING;"
            APPLIED=$((APPLIED + 1))
        else
            FAILED=$((FAILED + 1))
    ...
    # MIGRATION_FAIL_FAST 모드: 마이그레이션 실패 시 앱 시작 차단
    if [ "${MIGRATION_FAIL_FAST:-false}" = "true" ] && [ "$FAILED" -gt 0 ]; then
        echo "[entrypoint] ERROR: ${FAILED} migration(s) failed and MIGRATION_FAIL_FAST=true. Aborting."
        exit 1
    fi
fi
...
replace_env_placeholder() {
    ...
    if [ -n "$value" ] && [ "$value" != "$placeholder" ]; then
        ...
        if [ -d "/app/.next" ]; then
            find /app/.next -name '*.js' -exec sed -i "s|${placeholder}|${value}|g" {} + 2>/dev/null
            echo "[entrypoint] Replaced ${placeholder} → (set)"
        else
            echo "[entrypoint] WARNING: /app/.next directory not found, skipping placeholder replacement for ${placeholder}"
        fi
    fi
}
