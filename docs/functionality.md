...
- **알람 스케줄링**: DB에 `process_alarms_atomically` 함수가 준비되어 있으며, 스케줄러(v2)는 현재 개발 중입니다. 최초 알람과 12시간 이내 예정된 알람을 처리 대상으로 합니다 (과거 알람 포함)
...
- **Corrective RAG 파이프라인**: RAG 검색 결과의 품질을 자동으로 평가하고 개선
    - **관련성 평가**: 규칙 기반(유사도 점수 + 키워드 오버랩)으로 검색 결과 관련성 평가
    - **Adaptive 라우팅**: 평가 결과에 따라 검색 결과 활용/재검색/웹 검색 등 자동 결정
    - **클라이언트 사이드 실행**: Strategy 패턴으로 `getSimilarity`를 검색 함수로 주입
    - **환경변수로 제어**: `NEXT_PUBLIC_CRAG_ENABLED`(클라이언트), `CRAG_ENABLED`(서버)

### AI 트레이싱 및 메트릭 (Langfuse)

- **AI 호출 트레이싱**: LLM 호출의 입출력, 토큰 사용량, 지연시간 자동 추적
    - **Fire-and-forget 패턴**: 트레이싱이 앱 응답 시간에 영향 없음
    - **자동 비활성화**: Langfuse 키 미설정 시 no-op으로 동작
    - **메트릭 수집**: 채팅, 임베딩, CRAG 평가 등 AI 기능별 성능 데이터 수집
...
- 앱 설치 가능 (manifest.json)
- 개발자 모드: 로고 영역을 5번 연속 탭하여 전체 캐시 수동 삭제 가능

> **참고**: 오프라인 사용 및 푸시 알림 기능은 현재 미구현 상태입니다.
